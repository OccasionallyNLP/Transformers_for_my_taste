...
from .generation_logits_process import (
    ...
    HistoryNoRepeatNGramLogitsProcessor, # XXX
    TopKLogitsWarper,
    TopPLogitsWarper,
)
...
class GenerationMixin:
    ...
    def _get_logits_processor(
        ...
        history_no_repeat_ngram_size: int,
        ...
        history_no_repeat_ngram_size = (
            history_no_repeat_ngram_size
            if history_no_repeat_ngram_size is not None
            else self.config.history_no_repeat_ngram_size
        ) # XXX
        ...
        if history_no_repeat_ngram_size is not None and history_no_repeat_ngram_size > 0: # XXX
            if self.config.is_encoder_decoder: # XXX
                processors.append(HistoryNoRepeatNGramLogitsProcessor(history_no_repeat_ngram_size, history_input_ids)) # XXX
            else: # XXX
                raise ValueError( # XXX
                    "It's impossible to use `history_no_repeat_ngram_size` with decoder-only architecture" # XXX
                )         # XXX
    ...
    def generate(
        ...
        history_input_ids: Optional[torch.LongTensor] = None, # XXX
	history_no_repeat_ngram_size: Optional[int] = None,
        ...
        **model_kwargs,
        ...
        logits_processor = self._get_logits_processor(
            ...
            history_input_ids=history_input_ids, # XXX
            history_no_repeat_ngram_size=history_no_repeat_ngram_size, # XXX
